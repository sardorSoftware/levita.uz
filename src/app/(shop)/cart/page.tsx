...
// --- Rasm Slayderi uchun alohida ichki komponent ---
const CartItemCard = ({ item, updateQuantity, removeItem }: any) => {
    const [currentImageIndex, setCurrentImageIndex] = useState(0);
    
    // Store'dan kelgan rasmlarni xavfsiz ajratib olamiz
    const itemImages = item.images && Array.isArray(item.images) && item.images.length > 0 
    ? item.images 
    : (item.image ? [item.image] : []);
    
    // Time slide (avtomatik varoqlash) logikasi
    useEffect(() => {
        if (itemImages.length <= 1) return; // Rasm bitta bo'lsa aylantirmaymiz
        
        const timer = setInterval(() => {
            setCurrentImageIndex((prev) => (prev + 1) % itemImages.length);
        }, 3000); // Har 3 soniyada keyingi rasmga o'tadi
        
        return () => clearInterval(timer);
    }, [itemImages.length]);
    
    return (
        <div className="bg-white p-3 rounded-2xl flex gap-4 shadow-sm border border-gray-100 relative hover:shadow-md transition-shadow">
        {/* Rasm Slayder qismi */}
        <div className="w-24 h-24 bg-gray-50 rounded-xl flex-shrink-0 relative overflow-hidden flex items-center justify-center group">
        {itemImages.length > 0 ? (
            <>
            <Image 
            ...
            {itemImages.length > 1 && (
                ...
                {itemImages.map((_, idx) => (
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${
                        ...
                        ? "w-3 bg-primary" // Faol nuqta (apelsin rang)
                        : "w-1.5 bg-gray-300 opacity-70" // Nofaol nuqta
                        }`} 
                        />
                    ))}
                    </div>
            ...
            <h3 className="text-sm font-semibold text-dark line-clamp-2 pr-16">{item.title}</h3>
            
            {/* O'ng ustki qismdagi tugmalar */}
            ...
            className="p-2 text-gray-400 hover:text-red-500 bg-gray-50 hover:bg-red-50 rounded-full transition-colors cursor-pointer shadow-sm"
            ...
            {/* Miqdor nazorati */}
            <div className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-lg px-2 py-1">
            <button 
            type="button"
            onClick={() => {
                if (item.quantity > 1) {
            ...
            className="p-1 text-gray-500 hover:text-dark cursor-pointer transition-colors"
            >
